...
# Optional helper (example loader)
try:
    from load_pet_state import load_pet_state  # convenience for __main__ demo
except Exception:  # pragma: no cover
    load_pet_state = None  # type: ignore
...
def plan_slots_agent(state: nppState) -> nppState:
    """LLM-planned slots written into state.slots."""
    ctx = agent_helper_function.pet_context_helper(state.pet)
    raw = _call_llm(ctx)
...
def build_graph():
    ...
    graph.add_edge("plan_slots_agent", END)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--pet-id", type=int, default=1, help="Example pet id (1..n)")
    ...
    print("Planned slots (", len(out.slots), "):", sep="")
