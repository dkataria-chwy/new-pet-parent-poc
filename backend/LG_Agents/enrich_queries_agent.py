...
# Optional helper (for __main__ demo parity with slot_planner_agent.py)
try:
    from load_pet_state import load_pet_state  # convenience
...
def enrich_queries_agent(state: nppState) -> nppState:
    """
    ...
    pet_ctx = agent_helper_function.pet_context_helper(state.pet)
    slots_ctx = _slots_brief(state.slots)
